...
def key_class(updates, image_strip, image_rects, is_white_key=True):
    ...
    updates - a set into which a key instance adds itself if it needs
        redrawing.
    image_strip - The surface containing the image of all key states.
    ...
    depressed). Corresponding up and down image are drawn for each
    ...
    its neighbors. So a key may support multiple image and states
    ...
    # Images:
    #
    # A black key only needs two image, for the up and down states. Its
    ...
    # image, for self up, self down, and both self and adjacent white down.
    #
    # A white key with both a black and white key to its right needs six
    # image: self up, self up and adjacent black down, self down, self and
...
def key_images():
    ...
    image depending on the state of adjacent keys to its right.
