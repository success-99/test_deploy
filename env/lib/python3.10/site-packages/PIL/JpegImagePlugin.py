...
# 1997-08-27 fl   Save mode 1 image as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        msg = f"cannot handle {self.layers}-layer image"
        raise SyntaxError(msg)
...
##
# Image plugin for JPEG and JFIF image.
...
def _getmp(self):
    ...
    # it's an error not to have a number of image
    try:
        quant = mp[0xB001]
    except KeyError as e:
        msg = "malformed MP Index (no number of image)"
        raise SyntaxError(msg) from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    ...
    # (grayscale image) or when they are CMYK (4 layers),
